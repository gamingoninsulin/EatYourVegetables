package me.gamingoninsulin.eatyourvegtablesboymod.blocks.entity;

import me.gamingoninsulin.eatyourvegtablesboymod.blocks.custom.kitchen.KitchenOvenBlock;
import me.gamingoninsulin.eatyourvegtablesboymod.blocks.entity.util.ImplementedInventory;
...
import me.gamingoninsulin.eatyourvegtablesboymod.screen.KitchenOvenScreenHandler;
...
public class KitchenOvenStationBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory, ImplementedInventory {
    ...
    public KitchenOvenStationBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.KITCHEN_OVEN_STATION_BE, pos, state);
        this.propertyDelegate = new PropertyDelegate() {
            @Override
            public int get(int index) {
                return switch (index) {
                    case 0 -> KitchenOvenStationBlockEntity.this.progress;
                    case 1 -> KitchenOvenStationBlockEntity.this.maxProgress;
                    default -> 0;
                };
            }
            ...
            public void set(int index, int value) {
                switch (index) {
                    case 0: KitchenOvenStationBlockEntity.this.progress = value;
                    case 1: KitchenOvenStationBlockEntity.this.maxProgress = value;
                }
            }
    ...
    public void setInventory(DefaultedList<ItemStack> list) {
        for(int index = 0; index < list.size(); index++) {
            this.inventory.set(index, list.get(index));
        }
    }
    ...
    public final SingleVariantStorage<FluidVariant> fluidStorage = new SingleVariantStorage<FluidVariant>() {
        ...
        protected long getCapacity(FluidVariant variant) {
            return (FluidConstants.BUCKET / 81) * 8; // 1 Bucket = 81000 Droplets = 1000mB || *64 ==> 64,000mB = 64 Buckets
        }
    ...
    public boolean canInsert(int slot, ItemStack stack, @Nullable Direction side) {
        Direction localDir = this.getWorld().getBlockState(pos).get(KitchenOvenBlock.FACING);
    ...
    public boolean canExtract(int slot, ItemStack stack, Direction side) {
        Direction localDir = this.getWorld().getBlockState(this.pos).get(KitchenOvenBlock.FACING);
    ...
    public ScreenHandler createMenu(int syncId, PlayerInventory playerInventory, PlayerEntity player) {
        return new KitchenOvenScreenHandler(syncId, playerInventory, this, propertyDelegate);
    }
    ...
    public void writeNbt(NbtCompound nbt) {
        ...
        nbt.putInt("kitchen_oven_station.progress", progress);
        nbt.putLong(("kitchen_oven_station.energy"), energyStorage.amount);
        nbt.put("kitchen_oven_station.variant", fluidStorage.variant.toNbt());
        nbt.putLong("kitchen_oven_station_fluid.amount", fluidStorage.amount);
    }

    @Override
    public void readNbt(NbtCompound nbt) {
        Inventories.readNbt(nbt, inventory);
        progress = nbt.getInt("kitchen_oven_station.progress");
        energyStorage.amount = nbt.getLong("kitchen_oven_station.energy");
        fluidStorage.variant = FluidVariant.fromNbt((NbtCompound) nbt.get("kitchen_oven_station.variant"));
        fluidStorage.amount = nbt.getLong("kitchen_oven_station_fluid.amount");
        super.readNbt(nbt);
    }
    ...
    private void extractFluid() {
        try(Transaction transaction = Transaction.openOuter()) {
            this.fluidStorage.extract(FluidVariant.of(Fluids.WATER), 1000, transaction);
            transaction.commit();
        }
    }
    ...
    private boolean hasRecipe() {
        ...
        return canInsertAmountIntoOutputSlot(output.getCount())
                && canInsertItemIntoOutputSlot(output) && hasEnoughFluidToCraft(); // && hasEnoughEnergyToCraft();
    }
    ...
    private Optional<KitchenOvenRecipe> getCurrentRecipe() {
        SimpleInventory inventory = new SimpleInventory((this.size()));
        for(int i = 0; i < this.size(); i++) {
            inventory.setStack(i, this.getStack(i));
        }

        return this.getWorld().getRecipeManager().getFirstMatch(KitchenOvenRecipe.Type.INSTANCE, inventory, this.getWorld());
    }
