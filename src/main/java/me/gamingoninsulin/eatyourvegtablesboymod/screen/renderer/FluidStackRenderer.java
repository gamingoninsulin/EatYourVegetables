package me.gamingoninsulin.eatyourvegtablesboymod.screen.renderer;
...
public class FluidStackRenderer {
    ...
    public List<Text> getTooltip(SingleVariantStorage<FluidVariant> fluidStorage, TooltipContext tooltipFlag) {
        ...
        if (tooltipMode == TooltipMode.SHOW_AMOUNT_AND_CAPACITY) {
            MutableText amountString = Text.translatable("tooltip.eyvb.liquid.amount.with.capacity", nf.format(amount), nf.format(capacityMb));
            tooltip.add(amountString.fillStyle(Style.EMPTY.withColor(Formatting.DARK_GRAY)));
        } else if (tooltipMode == TooltipMode.SHOW_AMOUNT) {
            MutableText amountString = Text.translatable("tooltip.eyvb.liquid.amount", nf.format(amount));
            tooltip.add(amountString.fillStyle(Style.EMPTY.withColor(Formatting.DARK_GRAY)));
        }
